...
def sample_points(
    ...
    if crop_meshes:
        cube = trimesh.creation.box(extents=[1.0+padding, 1.0+padding, 1.0+padding])
        if isinstance(mesh, list):
            for idx in range(len(mesh)):
                mesh[idx] = mesh[idx].slice_plane(cube.facets_origin, -cube.facets_normal)
        else:
...
def process(f_in, f_out, n_points=500000, overwrite=False):
    # Load meshes
