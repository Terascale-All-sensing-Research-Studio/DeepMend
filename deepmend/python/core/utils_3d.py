...
def trimesh_boolean(mesh1, mesh2, operation="difference", **kwargs):
    return pymesh2trimesh(
        pymesh.boolean(trimesh2pymesh(mesh1), trimesh2pymesh(mesh2), operation, **kwargs)
    )
...
def points_transform(points, mat):
    ...
    points = np.dot(
        mat, np.hstack((points, np.ones((points.shape[0], 1)))).T
    ).T[:, :3]
    return points
...
def triangulate_plane(plane):
    ...
    return trimesh.Trimesh(
        vertices, 
        matplotlib.tri.Triangulation(x, y).triangles
    )
...
def colorize_mesh_from_index_auto(mesh, shape_idx, color_strength=60, transparent=False):
    """
    Applies a predefined color to a mesh based on its shape index, in place
    """
