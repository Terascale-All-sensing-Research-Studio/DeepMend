...
def saver(f_out, data, all_access=True, **kwargs):
    ...
    try:
        if extension == ".pkl":
            ...
            return json.dump(data, open(f_out, "r"))
        elif extension == ".ply":
            data.export(f_out)
        elif extension == ".npz":
            np.savez(f_out, data)
...
def execute_and_save(
    ...
    **kwargs,
    ...
    try:
        ...
        # raise e
        logging.error("Problem running: {}[{}] with save path: {}".format(str(fn), e, path))
    return val
                    ...
                    if shape_idx in outputs:
                        ...
                        if reconstruction_handler is not None:
                            ...
                            if not path_exists(path, p_list) or overwrite:
                                ...
                                # Update the colors
                                core.utils_3d.colorize_mesh_from_index_auto(mesh, shape_idx)
        ...
        for idx in tqdm.tqdm(reconstruct_list):
            ...
            for (gt_idx, pd_idx) in output_pairs:
                ...
                if metric in [
                    ...
                    "connected_artifacts_error", 
                    "chamfer_join",
                    "join_dissimilarity", 
                    "exterior_error",
                    "break_error", 
                    "close_exterior_error",
                    "join_similarity",
                    "total_exterior_error",
                    "total_break_error", 
                    "total_close_exterior_error",
                ] and pd_idx < 2:
                    continue
                # complete-broken-only metrics
                elif metric in ["complete_sameness"] and pd_idx >= 2:
                    continue
                ...
                if metric in ["connected_artifacts_score"]:
                ...
                if metric in ["connected_artifacts_score2", "chamfer_join", "join_dissimilarity", "join_similarity", "connected_artifacts_error"]:
                ...
                elif metric in ["break_error", "exterior_error", "close_exterior_error", "total_exterior_error", "total_break_error", "total_close_exterior_error"]:
                ...
                elif metric in ["chamfer_complete"]:
                ...
                elif metric in ["non_protrusion_score"]:
                ...
                elif metric in ["fractured_protrusion_score"]:
                    ...
                    "nested_binner",
                    "truth_tabler",
                ]:  # These are computed during evaluation
                    continue
...
def load_knittable(path):
    ...
    root = os.environ["DATADIR"]
    if "DATACENTER2" in path:
        root = os.environ["DATADIR2"]
...
class ReconstructionHandler:
    ...
    def path_mesh_interp(self, idx_from, idx_to, step, shape_idx, interp_type, create=False):
        return os.path.join(
            ws.get_reconstructions_meshes_dir(
                self._experiment_directory,
                name=self._name,
    ...
    def set_mesh_interp(
        ...
        path = self.path_mesh_interp(idx_from, idx_to, step, shape_idx, interp_type, create=True)
        if save:
            saver(path, mesh)
        self._cache[path] = mesh
