...
class LoadModel(object):
    def __init__(self, data_file, out_path, batch_size=64, num_workers=-1,
                 ...
                 evaluate_mode=False, fracture_data=True, shuffle_data=True, frac_ratio=None):
        '''
        inputs:
        ...
        self.fracture_data = fracture_data
        self.opt = opt.lower()
    ...
    def check_data_file(self):
        if os.path.exists(self.data_file) and self.data_file[-4:] == '.npy':
            ...
            raise RuntimeError("Data file does not exist: {}".format(self.data_file))
    ...
    def load_data(self, force=False):
        ...
        loader = CustomLoader(self.data_file, self.out_path, 'train', fracture=self.fracture_data, frac_ratio=self.frac_ratio)
        self.len_train = len(loader)
        
        if self.gan:
            # This is VITALLY MISSING from the default implementation
            assert len(loader) > self.minibatches_size, (
                "Too few samples to train discriminator for a single iteration. Num samples: {}, minibatch size: {}"
                .format(len(loader), self.minibatches_size)
                )
            self.d_loader = torch.utils.data.DataLoader(loader, self.minibatches_size, shuffle=self.shuffle_data,
                                                        num_workers=self.num_workers)
            print("Loaded Descriminator dataloader with {} batched samples ({} random samples will be skipped each iteration)".format(len(self.d_loader), len(loader) % self.minibatches_size))
        
        # This is VITALLY MISSING from the default implementation
        assert len(loader) > self.batch_size, (
            "Too few samples to train GAN for a single iteration. Num samples: {}, batch size: {}"
            .format(len(loader), self.batch_size)
            )
        self.g_loader = torch.utils.data.DataLoader(loader, self.batch_size, shuffle=self.shuffle_data,
                                                    num_workers=self.num_workers)
        self._load_n_labels()
        print("Loaded GAN dataloader with {} batched samples ({} random samples will be skipped each iteration)".format(len(self.g_loader), len(loader) % self.batch_size))
    ...
    def _load_full_train_set(self, **kwargs):
        data_train = CustomLoader(self.data_file, self.out_path, subset='train', fracture=self.fracture_data, frac_ratio=self.frac_ratio, **kwargs)
        len_train = len(data_train)
        train_loader = torch.utils.data.DataLoader(data_train, len_train, shuffle=self.shuffle_data,
                                                         num_workers=self.num_workers)
    ...
    def _load_full_test_set(self, **kwargs):
        data_test = CustomLoader(self.data_file, self.out_path, subset='test', fracture=self.fracture_data, frac_ratio=self.frac_ratio, **kwargs)
        len_test = len(data_test)
        test_loader = torch.utils.data.DataLoader(data_test, len_test, shuffle=self.shuffle_data,
                                                  num_workers=self.num_workers)
    ...
    def train(self, epochs, test_each=5, extend_training=True, max_extend=10):
        ...
        iters = max(1, int(self.len_train // self.minibatches_size))
        left_epochs = epochs
    ...
    def _evaluate_and_log(self, epoch):
        ...
        # write_training_log(self.training_log_file, epoch+1, [train_loss])
