...
def reconstruct(
    ...
    # Get the points
    pts, data_sdf, mask, sample_kwargs = test_sdf
    ...
    if slippage_method == "fake-classifier":
        mask = core.data.partial_sample_with_classification_noise(
            ...
            pts=sample_kwargs["pts"],
            mask=sample_kwargs["mask"],
...
if __name__ == "__main__":
    ...
    L2019_IDX = 15
    L2019_WATERPROOF_IDX = 17

    network_outputs = 0
    total_outputs = (0, 2, 9, L2019_IDX, L2019_WATERPROOF_IDX)
    composite = [
        (1, 2),
        (1, 9),
        (1, L2019_IDX),
    ...
    if not args.skip_generate:
        ...
        for ii in reconstruct_list:
            ...
            if (not os.path.exists(path_code)) or overwrite_codes:
                pts, data_sdf, mask = sdf_dataset.get_broken_sample(ii, return_mask=True)
                input_list.append(
                    dict(
                        test_sdf=[
                            pts,
        ...
        with multiprocessing.Pool(render_threads) as pool:
            ...
            # Create the restoration meshes, with a threshold
            input_list, path_list = [], []
            for ii in reconstruct_list:
    ...
    path = os.path.join(
        reconstruction_handler.path_reconstruction(),
        "summary_img_{}.jpg",
    )
    logging.info(
